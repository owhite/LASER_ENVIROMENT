...
class Gcode:
    def __init__(self, cfg=None):
        ...
        self.units = Rhino.RhinoDoc.ActiveDoc.ModelUnitSystem
        ...
        if not self.EstablishMount(path):
            ...
            # note of stupidity -- the user always assigns feed rate in inches
            #  but it will be converted to mm/sec if the document units are millimeters
            if self.units == Rhino.UnitSystem.Millimeters:
                print("inch-based feed rate %s converted to mm %s") % (self.move_feed_rate, int(int(self.move_feed_rate) * 25.4))
                self.move_feed_rate = int(int(self.move_feed_rate) * 25.4)
    ...
    def AddHeader(self):
        # Print the units
        print("current document uses %s") % (self.units)

        if self.units == Rhino.UnitSystem.Inches:
            first_str = 'G17 G20 G40 G49 G80 G90\n'
            print "gcode G20, inches"
        elif self.units == Rhino.UnitSystem.Millimeters:
